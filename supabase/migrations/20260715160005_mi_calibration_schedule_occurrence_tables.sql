-- Mechanical Integrity Phase 7 - calibration schedule and occurrences.

create table if not exists public.mi_calibration_plan_schedules (
...
create table if not exists public.mi_calibration_occurrences (
  id text primary key default gen_random_uuid()::text,
  company_id text not null,
  site_id text not null,
  plan_id text not null references public.mi_calibration_plans(id) on delete cascade,
  ...
  superseded_by_occurrence_id text references public.mi_calibration_occurrences(id),
