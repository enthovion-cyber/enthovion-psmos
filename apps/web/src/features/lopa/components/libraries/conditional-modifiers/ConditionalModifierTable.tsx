...
import type { ConditionalModifierLibraryRecord, LopaLibraryResponse } from '../../../types/lopa-library.types';
import { LibraryScopeBadge, LibraryStatusBadge } from '../LibraryShared';

export function ConditionalModifierTable({
  data,
  isLoading,
  onView,
  onEdit,
  ...
  data: LopaLibraryResponse<ConditionalModifierLibraryRecord> | undefined;
  isLoading?: boolean;
  onView: (record: ConditionalModifierLibraryRecord) => void;
  onEdit: (record: ConditionalModifierLibraryRecord) => void;
  onAction: (record: ConditionalModifierLibraryRecord, action: 'submit-review' | 'approve' | 'reject' | 'create-revision' | 'archive') => void;
}) {
  const rows = data?.rows ?? [];
  if (isLoading) return <div className="rounded-xl border border-cyan-300/10 bg-[#071525] p-8 text-center text-slate-300">Loading conditional modifier library...</div>;
  if (!rows.length) return <div className="rounded-xl border border-cyan-300/10 bg-[#071525] p-8 text-center text-slate-400">No conditional modifier records match the current filters.</div>;
  return (
    <section className="overflow-hidden rounded-xl border border-cyan-300/10 bg-[#071525]">
      <div className="overflow-x-auto">
        <table className="min-w-[1180px] w-full text-left text-sm">
          <thead className="sticky top-0 bg-[#0b1d31] text-[11px] uppercase tracking-wide text-slate-400">
            <tr>
              {['Modifier', 'Type / Context', 'Value', 'Approved Range', 'Source', 'Scope', 'Approval', 'Revision', 'Actions'].map((head) => <th key={head} className="border-b border-cyan-300/10 px-4 py-3">{head}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.map((record) => (
              <tr key={record.id} className="border-b border-cyan-300/10 text-slate-200 hover:bg-cyan-300/5">
                <td className="px-4 py-3">
                  <button onClick={() => onView(record)} className="font-black text-blue-300 hover:text-blue-200">{record.modifier_code}</button>
                  <div className="mt-1 font-semibold text-white">{record.modifier_name}</div>
                  <div className="line-clamp-1 text-xs text-slate-500">{record.description || 'No description'}</div>
                </td>
                <td className="px-4 py-3">
                  <div className="font-semibold">{record.modifier_type}</div>
                  <div className="line-clamp-1 max-w-[200px] text-xs text-slate-500">{record.application_context || '-'}</div>
                </td>
                <td className="px-4 py-3">
                  <div className="font-black text-amber-200">{Number(record.default_value).toPrecision(3)}</div>
                  <div className="text-xs text-slate-500">{record.unit || 'factor'}</div>
                </td>
                <td className="px-4 py-3">
                  <div>{record.low_value ?? '-'} / {record.high_value ?? '-'}</div>
      ...
      <div className="flex items-center justify-between border-t border-cyan-300/10 px-4 py-3 text-xs text-slate-400">
        ...
        <span>Study use snapshots value, range, source, and approval status.</span>
      </div>
    </section>
  );
}
