...
import { validatePssrTrainingWaiver } from '../schemas/pssr-training-waiver.schema';
import { TrainingButton, formatTrainingError } from '../shared/TrainingUi';

export function PssrTrainingWaiverDialog({ blockerId, onClose }: { blockerId: string; onClose?: () => void }) {
  const [values, setValues] = useState<Record<string, any>>({});
  const [errors, setErrors] = useState<string[]>([]);
  const mutations = usePssrTrainingMutations();
  async function submit() {
    const nextErrors = validatePssrTrainingWaiver(values);
