...
export function BlockerList({ rows }: { rows?: any[] | undefined }) {
  const safeRows = rows ?? [];
  if (!safeRows.length) return <TrainingEmptyState title="No blockers returned" message="Implementation, closure and startup blockers will appear here when backend readiness detects them." />;
...
export function RequirementLink({ row }: { row: any }) {
  return <Link className="font-semibold text-primary hover:underline" href={`/training-competency/moc-training-requirements/${row.id}`}>{valueText(row.requirement_title ?? row.requirement_code ?? row.id)}</Link>;
}
