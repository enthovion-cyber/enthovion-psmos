...
export function FieldBlockersPanel({ blockers }: { blockers: any[] }) {
  // 1. Manage visible state expansion
  ...
  return (
    <PSSRCard title={`Field Blockers (${blockers.length})`}>
      {blockers.length > 0 ? (
        <div className="space-y-4">
          ...
          {hasMoreItems && (
            <div className="mt-4 flex justify-center border-t border-white/5 pt-3">
              ...
              >
                <span>{isExpanded ? 'Show Less' : `View All Field Blockers (${blockers.length})`}</span>
        ...
        <EmptyState 
          title="No field blockers" 
          detail="Equipment not verified, tag mismatch, safety-critical failures, lineup issues, and missing photos appear here." 
        />
      )}
    </PSSRCard>
  );
