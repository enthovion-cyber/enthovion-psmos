...
@Injectable()
export class AuditFindingHistoryService {
  constructor(
  ...
  async write(input: Record<string, any>) {
    const event = {
      ...
      source_module: "Audit Finding Register",
      source_record_id: input.findingId ?? input.fieldFindingId ?? input.executionId ?? null,
    };
    await this.db.single(this.db.from("audit_finding_history_events").insert(event).select("id").single());
    await this.audit.write({
      tenantId: input.tenantId,
      actorId: input.actorId,
      action: input.type,
      entityType: "audit_finding",
      ...(input.findingId ? { entityId: input.findingId } : {}),
      ...(input.before ? { before: input.before as JsonValue } : {}),
      ...(input.after ? { after: input.after as JsonValue } : {}),
      metadata: { title: input.title, executionId: input.executionId ?? null } as JsonValue,
    });
    return event;
  }
}
