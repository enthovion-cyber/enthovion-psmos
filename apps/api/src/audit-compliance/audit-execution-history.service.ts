import { Injectable } from "@nestjs/common";
import { AuditService } from "../audit/audit.service";
import { JsonValue } from "../common/types/db.types";
import { SupabaseService } from "../database/supabase.service";

@Injectable()
export class AuditExecutionHistoryService {
  constructor(
    private readonly db: SupabaseService,
    private readonly audit: AuditService,
  ) {}
  async write(input: Record<string, any>) {
    const event = {
      company_id: input.tenantId,
      site_id: input.siteId ?? null,
      unit_id: input.unitId ?? null,
      area_id: input.areaId ?? null,
      program_id: input.programId ?? null,
      plan_id: input.planId ?? null,
      checklist_id: input.checklistId ?? null,
      execution_id: input.executionId ?? null,
      response_id: input.responseId ?? null,
      field_finding_id: input.findingId ?? null,
      event_type: input.type,
      event_title: input.title,
      event_description: input.description ?? input.title,
      before_value_json: input.before ?? null,
      after_value_json: input.after ?? null,
      actor_user_id: input.actorId,
      source_module: "Audit Execution",
      source_record_id: input.executionId ?? input.findingId ?? null,
    };
    await this.db.single(
      this.db
        .from("audit_execution_history_events")
        .insert(event)
        .select("id")
        .single(),
    );
    await this.db
      .single(
        this.db
          .from("audit_execution_activity_events")
          .insert({
            company_id: input.tenantId,
            site_id: input.siteId ?? null,
            unit_id: input.unitId ?? null,
            area_id: input.areaId ?? null,
            execution_id: input.executionId,
            response_id: input.responseId ?? null,
            field_finding_id: input.findingId ?? null,
            event_type: input.type,
            event_title: input.title,
            event_description: input.description ?? input.title,
            actor_user_id: input.actorId,
            source_module: "Audit Execution",
            source_record_id: input.executionId ?? input.findingId ?? null,
          })
          .select("id")
          .single(),
      )
      .catch(() => null);
    await this.audit.write({
      tenantId: input.tenantId,
      actorId: input.actorId,
      action: input.type,
      entityType: "audit_execution",
      ...(input.executionId ? { entityId: input.executionId } : {}),
      ...(input.before ? { before: input.before as JsonValue } : {}),
      ...(input.after ? { after: input.after as JsonValue } : {}),
      metadata: { title: input.title } as JsonValue,
    });
    return event;
  }
}
